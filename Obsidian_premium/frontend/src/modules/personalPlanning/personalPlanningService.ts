...
const API_PREFIX = '/api/personal-planning/cards'

class PersonalPlanningError extends Error {
  constructor(message: string) {
    super(message)
    this.name = 'PersonalPlanningError'
  }
}
...
async function request<T>(path: string, init?: RequestInit): Promise<T> {
  ...
  try {
    ...
    throw new PersonalPlanningError('Servidor indisponível no momento.')
  }

  if (!response.ok) {
    let message = response.status === 401 ? 'Token inválido ou ausente.' : 'Falha ao processar solicitação.'
    try {
    ...
    throw new PersonalPlanningError(message)
  }
  ...
  try {
    ...
    throw new PersonalPlanningError('Resposta inválida da API.')
  }
}
...
export async function createPlanningCard(input: PlanningCardCreateInput): Promise<PlanningCard> {
  ...
  if (!created) throw new PersonalPlanningError('Resposta inválida ao criar card.')
  return created
}
...
export async function updatePlanningCard(id: string, input: PlanningCardUpdateInput): Promise<PlanningCard> {
  ...
  if (!updated) throw new PersonalPlanningError('Resposta inválida ao atualizar card.')
  return updated
}
