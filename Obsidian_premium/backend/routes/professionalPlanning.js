...
router.get('/professional-planning/cards', (req, res) => {
  const db = req.app.locals.db
  const rows = db.prepare('SELECT * FROM planning_cards ORDER BY createdAt').all()
  res.json(rows.map(rowToCard))
})

router.post('/professional-planning/cards', (req, res) => {
  const db = req.app.locals.db
  ...
  db
    .prepare(
      'INSERT INTO planning_cards (id,title,description,status,priority,isFinalized,completedAt,createdAt,updatedAt) VALUES (@id,@title,@description,@status,@priority,@isFinalized,@completedAt,@createdAt,@updatedAt)'
    )
    .run(card)

  const row = db.prepare('SELECT * FROM planning_cards WHERE id = ?').get(card.id)
  res.status(201).json(rowToCard(row))
})

router.patch('/professional-planning/cards/:id', (req, res) => {
  const db = req.app.locals.db
  ...
  db
    .prepare(
      'UPDATE planning_cards SET title=@title, description=@description, status=@status, priority=@priority, isFinalized=@isFinalized, completedAt=@completedAt, updatedAt=@updatedAt WHERE id=@id'
    )
    .run(updated)

  const row = db.prepare('SELECT * FROM planning_cards WHERE id = ?').get(id)
  res.json(rowToCard(row))
})

router.delete('/professional-planning/cards/:id', (req, res) => {
  const db = req.app.locals.db
  const info = db.prepare('DELETE FROM planning_cards WHERE id = ?').run(req.params.id)
